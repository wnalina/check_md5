...
functions = ['open_camera', 'azure_camera', 'list_group', 'list_person',
        ...
        if len(set(faceFoundArr)) <= 1:
            ...
            if confirmFound != allArrLastFound:
                ...
                if int(allArrLastFound) >= 1:
                    ...
                    # print("face_ids count = " + str(azureFound))
                    if azureFound >= 1:
                        identified_faces = CF.face.identify(face_ids, 'myteams')
                        print('azureFound = ' + str(azureFound))
                        # print('identified_faces' + str(identified_faces))
                        for count in range(azureFound):
                            candidate = identified_faces[count]['candidates']

                            if len(identified_faces[0]['candidates']) == 0:
                                print('\tname: unknown')
                                continue
...
def detection():
    ...
    for identified_face in identified_faces:
         ...
         if candidates :
             ...
             person_lists = personIds
             count_person = len(person_lists)
             # print(range)
             for i in range(count_person):
                 id = person_lists[i]['personId']
                 print(id)
                 # if  personId == id:
...
def listFunction():
    lists = ['open_camera', 'azure_camera', 'list_group', 'list_person',
                 'create_group', 'create_person_group', 'detection', 'sync_person', 'quit']
    i = 1
    cprint('List Function', 'blue')
    for alist in lists:
        cprint('\t{} '.format(i) + '{}'.format(alist), 'red')
