                ...
                if azureFound >= 1:
                    ...
                    for count in range(azureFound):
                        ...
                        if not candidate:  # check empty list
                            ...
                            if len(info_azure):
                                for person in info_azure:
                                    if candidate_personId == person.id:
                                        now = datetime.datetime.now().replace(microsecond=0)
                                        diff = now - person.time
                                        minutes = diff.total_seconds() / 60
                                        if minutes < 1:
                                            print('if')
                                            person.time = now
                                            cprint('id: ' + person.id, 'yellow')
                                            cprint('round: ' + str(person.round), 'yellow')

                                        else:
                                            print('else')
                                            person.round = person.round + 1
                                            person.time = now
                                            cprint('id: ' + person.id, 'yellow')
                                            cprint('round: ' + str(person.round), 'yellow')
                                        ...
                                        now = datetime.datetime.now().replace(microsecond=0)
                                        obj = IdAzure(candidate_personId, now, 0)
                                        info_azure.append(obj)
                                        print('add ' + str(candidate_personId) + ' complete')
                                        found -= 1
                            else:
                                now = datetime.datetime.now().replace(microsecond=0)
                                obj = IdAzure(candidate_personId, now, 0)
